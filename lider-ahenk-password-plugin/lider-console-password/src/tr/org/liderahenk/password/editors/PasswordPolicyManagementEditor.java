package tr.org.liderahenk.password.editors;
...
public class PasswordPolicyManagementEditor extends EditorPart {
	

	private static final Logger logger = LoggerFactory.getLogger(PasswordPolicyManagementEditor.class);
...
//					dialog.open();
	...
	private void createButtonsArea(final Composite parent) {
		...
		btnViewDetail.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				try {
					Command command = TaskRestUtils.getCommand(selectedTask.getId());
				//	AddPasswordPolicy2LDAPDialog dialog = new AddPasswordPolicy2LDAPDialog(Display.getDefault().getActiveShell(), command);
				//	dialog.create();
				//	dialog.open();
				} catch (Exception e1) {
					logger.error(e1.getMessage(), e1);
	...
	private void populateTable() {
		try {
			...
			tasks = TaskRestUtils.listExecutedTasks(PasswordConstants.PLUGIN_NAME, false, false, null, null, null,
