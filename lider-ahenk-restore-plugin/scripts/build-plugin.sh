...
# This script builds the project and generates Lider (lider-restore.jar) & Lider Console (lider-console-restore.jar) distribution files as well as Ahenk package (restore.deb)
#
# Generated files can be found under /tmp/lider-ahenk-restore-plugin/
set -e
...
cd "$PRJ_ROOT_PATH"/ahenk-restore
dpkg-buildpackage -b -uc
echo "Generated Ahenk package"

EXPORT_PATH=/tmp/lider-ahenk-restore-plugin
...
cp -rf "$PRJ_ROOT_PATH"/lider-restore/target/lider-*.jar "$EXPORT_PATH"
cp -rf "$PRJ_ROOT_PATH"/lider-console-restore/target/lider-console-*.jar "$EXPORT_PATH"
