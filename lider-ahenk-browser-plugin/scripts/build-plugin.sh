...
###
# This script builds the project and generates Lider (lider-browser.jar) & Lider Console (lider-console-browser.jar) distribution files as well as Ahenk package (browser.deb)
#
# Generated files can be found under /tmp/lider-ahenk-browser-plugin/
###
set -e
...
cd "$PRJ_ROOT_PATH"/ahenk-browser
dpkg-buildpackage -b -uc
echo "Generated Ahenk package"

EXPORT_PATH=/tmp/lider-ahenk-browser-plugin
...
cp -rf "$PRJ_ROOT_PATH"/lider-browser/target/lider-*.jar "$EXPORT_PATH"
cp -rf "$PRJ_ROOT_PATH"/lider-console-browser/target/lider-console-*.jar "$EXPORT_PATH"
