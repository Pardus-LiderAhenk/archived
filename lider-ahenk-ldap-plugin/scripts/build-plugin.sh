...
# This script builds the project and generates Lider (lider-ldap.jar) & Lider Console (lider-console-ldap.jar) distribution files as well as Ahenk package (ldap.deb)
#
# Generated files can be found under /tmp/lider-ahenk-ldap-plugin/
set -e
...
cd "$PRJ_ROOT_PATH"/ahenk-ldap
dpkg-buildpackage -b -uc
echo "Generated Ahenk package"

EXPORT_PATH=/tmp/lider-ahenk-ldap-plugin
...
cp -rf "$PRJ_ROOT_PATH"/lider-ldap/target/lider-*.jar "$EXPORT_PATH"
cp -rf "$PRJ_ROOT_PATH"/lider-console-ldap/target/lider-console-*.jar "$EXPORT_PATH"
